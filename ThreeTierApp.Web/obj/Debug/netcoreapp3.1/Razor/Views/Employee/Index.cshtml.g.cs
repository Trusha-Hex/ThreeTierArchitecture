#pragma checksum "E:\Trusha\Training\ThreeTierApp\ThreeTierApp.Web\Views\Employee\Index.cshtml" "{8829d00f-11b8-4213-878b-770e8597ac16}" "8a6dbaa13f35f0c3f004000da2754f13703cc2e673f5da87bfcb4cccc73f23bf"
// <auto-generated/>
...
namespace AspNetCoreGeneratedDocument
{
    ...
    #line default
    #line hidden
    [global::Microsoft.AspNetCore.Razor.Hosting.RazorSourceChecksumAttribute(@"Sha256", @"8a6dbaa13f35f0c3f004000da2754f13703cc2e673f5da87bfcb4cccc73f23bf", @"/Views/Employee/Index.cshtml")]
    #nullable restore
...
#nullable disable
            ...
            WriteLiteral("\r\n");
            __tagHelperExecutionContext = __tagHelperScopeManager.Begin("head", global::Microsoft.AspNetCore.Razor.TagHelpers.TagMode.StartTagAndEndTag, "8a6dbaa13f35f0c3f004000da2754f13703cc2e673f5da87bfcb4cccc73f23bf3116", async() => {
                WriteLiteral(@"
    <meta charset=""UTF-8"">
...
");
            ...
            WriteLiteral("\r\n\r\n");
            __tagHelperExecutionContext = __tagHelperScopeManager.Begin("body", global::Microsoft.AspNetCore.Razor.TagHelpers.TagMode.StartTagAndEndTag, "8a6dbaa13f35f0c3f004000da2754f13703cc2e673f5da87bfcb4cccc73f23bf10275", async() => {
                WriteLiteral(@"
    ...
    <div class=""modal-dialog modal-lg"">
        <div class=""modal-content"">
            <div class=""modal-header"">
                <h5 class=""modal-titl");
                WriteLiteral(@"e"" id=""tasksModalLabel"">Assigned Tasks</h5>
    ...
    <div class=""modal fade"" id=""employeeModal"" tabindex=""-1"" aria-labelledby=""employeeModalLabel"" aria-hidden=""true"">
        <div class=""modal-dialog modal-lg"">
            <div class=""modal-content"">
                ...
                <div id=""error-message"" style=""color: red; disp");
                WriteLiteral(@"lay: none;""></div>
                <div class=""modal-body"">
  ...
  ");
    ...
    <script>
            ...
            columns: [
                ...
                WriteLiteral(@"row) {
                        return `
                            <div class=""form-check form-switch"">
                                <input class=""form-check-input"" type=""checkbox"" id=""active-toggle-${row.id}"" ${data ? ""checked"" : """"} onchange=""toggleActive(${row.id}, this)"">
                            </div>
                        `;
                    }
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                            <button class=""btn btn-warning"" onclick=""editEmployee(${row.id})"">
                                <i class=""fas fa-edit""></i>
                            </button>
                            <button class=""btn btn-danger"" onclick=""deleteEmployee(${row.id})"">
                                <i class=""fas fa-trash""></i>
                            </button>
                            <button class=""btn btn-primary btn-sm"" data-bs-toggle=""modal"" data-");
                WriteLiteral(@"bs-target=""#tasksModal"" onclick=""setEmployeeIdForTasks(${row.id})"">
                                Tasks Assigned
                            </button>
                        `;
                    }
        ...
        function toggleActive(employeeId, toggle) {
            ...
            $.ajax({
                url: `https://localhost:65391/update-status/${employeeId}?isActive=${isActive}`, 
                method: 'PUT',
                ...
                error: function(error) {
                    ...
                    showAlert('danger', 'Failed to updat");
                WriteLiteral(@"e employee status.');
                }
            });
        }
        ...
        function showAlert(type, message) {
            ...
            alertBox.addClass(`alert alert-${type} alert-dismissible fade show`);
            alertBox.html(`
        ...
        $(""#add-btn"").click(function () {
            ...
            const department = parseInt");
                WriteLiteral(@"($(""#employee-department"").val());  // Get department as integer
            const salary = parseFloat($(""#employee-salary"").val());
            const role = parseInt($(""#employee-role"").val());  // Get role as integer

            let isValid = true;
            $("".error-message"").hide();

            if (!name || !username || !email || !password || isNaN(department) || isNaN(role) || isNaN(salary) || salary <= 0) {
                isValid = false;
            ...
            if (isValid) {
                const employee = {
                    ...
                    department,  // Send department as an integer
                    salary,
                    role,  // Send role as an integer
                };

                $.ajax({
            ");
                WriteLiteral(@"        url: apiBaseUrl,
                    type: ""POST"",
                    data: JSON.stringify(employee),
                    contentType: ""application/json"",
                    success: function (response) {
                        employeesTable.ajax.reload(); // Reload table data using DataTables
                        clearForm();
                        $(""#employeeModal"").modal(""hide""); // Close the modal
                        showAlert(""success"", response.message || ""Employee added successfully!""); // Success alert
                    },
                    error: function (xhr) {
                        const errorMessage = xhr.responseJSON?.message || ""Failed to add employee."";
                        showAlert(""danger"", errorMessage); // Error alert
                    },
                });
            }
        });
        ...
        $(""#update-btn"").click(function () {
            ...
            const name = $(""#employee-name"").val();
         ");
                WriteLiteral(@"   const username = $(""#employee-username"").val();
            const email = $(""#employee-email"").val();
            const password = $(""#employee-password"").val();
            const department = parseInt($(""#employee-department"").val()); // Ensure department is an integer
            const salary = parseFloat($(""#employee-salary"").val());
            const role = parseInt($(""#employee-role"").val()); // Ensure role is an integer

            let isValid = true;
            $("".error-message"").hide();

            // Validation for all fields and salary
            if (!name || !username || !email || !password || isNaN(department) || !department || isNaN(role) || isNaN(salary) || salary <= 0) {
                isValid = false;
                $(""#error-message"").text(""All fields are required, and salary must be greater than 0. Department and Role must be valid integers."").show();
            }

            if (isValid) {
                const employee = {
                    id: parseInt(id),");
                WriteLiteral(@"      // Employee ID as integer
                    name,
                    username,
                    email,
                    passwordHash: password,
                    department,            // Department as integer
                    salary,
                ...
                $.ajax({
                    ...
                    contentType: ""application/json"",
                    success: function () {
                        employeesTable.ajax.reload(); // Reload table data
                        clearForm(); // Clear the form fields
                        $(""#add-btn"").show(); // Show the ""Add"" button
                        $(""#update-btn"").hide(); // Hide the ""Update"" button
                        $(""#employeeModal"").modal(""hide""); // Close the modal
                        showAlert(""success"", ""Emp");
                WriteLiteral(@"loyee updated successfully!""); // Success alert
                    },
                    error: function (xhr) {
        ...
        function editEmployee(id) {
            $.get(`${apiBaseUrl}/${id}`, function(employee) {
                ...
                $(""#employee-role"").val(employee.role); // Set the role in the drop");
                WriteLiteral(@"down
        ...
        function deleteEmployee(id) {
            if (confirm(""Are you sure you want to delete this employee?"")) {
                $.ajax({
                    url: `${apiBaseUrl}/${id}`, // Ensure endpoint matches the API route
                    type: ""DELETE"",
                    success: function () {
                        // Refresh table
                        employeesTable.ajax.reload(); // Refresh DataTable directly
                        showAlert(""success"", ""Employee deleted successfully!"");
                    },
                    error: function (xhr) {
                        const errorMessage = xhr.responseJSON?.message || ""Failed to delete employee."";
                        console.error(""Delete Error:"", xhr); // Log the error for debugging
                        showAlert(");
                WriteLiteral(@"""danger"", errorMessage);
                    },
                });
            }
        }
        ...
        $('#tasksModal').on('show.bs.modal', function () {
            ...
            if (employeeId) {
                ...
                fetchAssignedTasks(employeeId);  // Fetch tasks using employeeId
            } else {
